 ...
 * @author Martin
 */
public class WaterNetworkFileGMSDAT {
    ...
    public DynamicNetwork<GasNode, GasEdge> readFromFile(String fileName) {
        ...
        for (i = 1; i < list.size(); i++) {
            if (list.get(i).contains("param: NODE") || list.get(i).contains("demand") || list.get(i).contains("Table ndata")) {
                continue;
            }
            ...
            if (list.get(i).contains("param: PIPE")) {
                break;
            }            
        ...
        for (i++; i < list.size(); i++) {
            ...
            String line = list.get(i).replace(";","").trim();
            ...
            assert fnode != null : "F: " + f + " " + Arrays.toString(parts) + " " + stringToNode;
            assert tnode != null : "T: " + t + " " + Arrays.toString(parts); 
        ...
        } else if (list.get(i).startsWith("set source:=")) {
            int first = line.indexOf("=")+1;
            String id = line.substring(first, line.indexOf(";")).trim();
            if (nodeTypes.get(stringToNode.get(id)).equals(IntersectionType.NODE)) {
                numberOfTerminals++;
            }
            nodeTypes.put(stringToNode.get(id), IntersectionType.SOURCE);
